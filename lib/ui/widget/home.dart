...
class _HomeState extends State<Home> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        _login(),
        _mainCircle(height * 0.26),
        _commentary(height * 0.1),
        Expanded(
            child: Column(
              ...
              children: [
                ...
                _service.userInfo.value.userid == null
                    ...
                    : _fbService.top10Friends.isEmpty
                      ? _top10Friends(height * 0.4)
                      : _top10(height * 0.4, _fbService.top10Friends)
              ],
            )
        ),
      ],
  ...
  Widget _commentary(double height) {
  ...
  // Future<String> _readJournal() async {
  //   return await rootBundle.loadString(kConst.journalPath);
  // }

  Widget _mainCircle(double height) {
    double rSize = height;
    int days = 0;
    int streak = 0;
    int kcalTotal = 0;
    if (_service.userInfo.value.totalDays != null) {
      days = _service.userInfo.value.totalDays!;
    }
    if (_service.userInfo.value.streakDays != null) {
      streak = _service.userInfo.value.streakDays!;
    }
    if (_service.userInfo.value.totalKcal != null) {
    ...
    return ElevatedButton(
      ...
      child: Column(
        ...
        children: [
          ...
          FittedBox(
            fit: BoxFit.scaleDown,
            child: Text('$kcalTotal kcal', style: TextStyle(fontSize: 18, color: kColor.circleText,
                fontWeight: FontWeight.w500),),
          ),
        ],
      ),
    ...
    return FutureBuilder(
        ...
        builder: (BuildContext context, AsyncSnapshot snapshot) {
          if (snapshot.hasData) {
            ...
            if (users.isNotEmpty) {
              return _top10(height, users); // return list<UserInfo>
            } else {
              return Padding(
                padding: const EdgeInsets.only(top: 100),
  ...
  Widget _top10(height, List<UserModel> users) {
    double width = MediaQuery.of(context).size.width;
    ...
    return Container(
      ...
      child: ListView.builder(
          ...
          itemBuilder: (BuildContext context, int index) {
            ...
            return GestureDetector(
              onTap: () => dialogSendMessage(context, users[index].userid!),
              child: Container(
                height: 55,
                color: kColor.decorationBox,
                alignment: Alignment.centerLeft,
